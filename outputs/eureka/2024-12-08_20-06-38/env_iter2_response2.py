...
def compute_reward(internal_state: np.ndarray) -> Tuple[np.float64, bool, bool, Dict[str, np.float64]]:
    ...
    # Encourage faster movement between thresholds
    movement_reward = 0.0
    if cart_position <= -swing_threshold and touched_left:
        movement_reward += np.abs(cart_velocity)
    elif cart_position >= swing_threshold and touched_right:
        movement_reward += np.abs(cart_velocity)

    # Transformations with adjusted parameters
    pole_temp = 0.2
    move_temp = 0.3

    transformed_pole_balance_reward = np.exp(pole_temp * pole_balance_reward) - 1
    transformed_movement_reward = np.exp(move_temp * movement_reward) - 1

    # Total reward
    reward = np.float64(transformed_pole_balance_reward + transformed_movement_reward)
