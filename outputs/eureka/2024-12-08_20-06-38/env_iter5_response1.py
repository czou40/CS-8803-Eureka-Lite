...
def compute_reward(internal_state: np.ndarray) -> Tuple[np.float64, bool, bool, Dict[str, np.float64]]:
    ...
    pole_balance_scale = 2.0
    pole_balance_temp = 0.05

    # Cart movement reward: Encourage crossing thresholds
    cart_movement_reward = 0.1 if (touched_left or touched_right) else 0.0
    cart_movement_scale = 1.0
    cart_movement_temp = 0.05

    # Pole velocity penalty: Discourage rapid pole swings
    pole_velocity_penalty = -0.01 * abs(pole_angular_velocity)
