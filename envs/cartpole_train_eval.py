...
def main():
    ...
    if not args.is_eval:
        ...
        # take five frames from a random episode
        test_env = CustomCartPoleEnv(render_mode='rgb_array')

        print("Visualizing a few frames .")
        
        obs, info = test_env.reset()
        done, truncated = False, False
        frames = []
        while not (done or truncated):
            frame = test_env.render()  # Render the environment step
            frames.append(frame)
            action, _ = model.predict(obs, deterministic=True)
            obs, reward, done, truncated, info = test_env.step(action)

        # Now we have a list of frames in `frames`
        num_frames = len(frames)

        if num_frames > 0:
            # Calculate the indices for the desired frames
            first_idx = 0
            last_idx = num_frames - 1
            quarter_idx = num_frames // 4  # 25% frame
            half_idx = num_frames // 2     # 50% frame
            three_quarter_idx = (3 * num_frames) // 4  # 75% frame

            # Indices we want to save
            indices_to_save = [first_idx, quarter_idx, half_idx, three_quarter_idx, last_idx]

            # Save the selected frames as jpg
            for i, idx in enumerate(indices_to_save):
                frame_array = frames[idx]
                img = Image.fromarray(frame_array)
                img.save(f"iter{args.iter}_response{args.response_id}_frame_{i}.jpg")
        ...
        video_env = RecordVideo(
            CustomCartPoleEnv(render_mode='rgb_array'), 
            video_folder="./videos", 
            name_prefix=f"iter{args.iter}_response{args.response_id}_video",
            episode_trigger=lambda episode_id: True,  # Record every episode for demonstration
        )

        obs, info = video_env.reset()
        done, truncated = False, False

        # Run one episode for the video
        while not (done or truncated):
            action, _ = model.predict(obs, deterministic=True)
            obs, reward, done, truncated, info = video_env.step(action)

        video_env.close()
        print("Video recorded. Check the ./videos directory.")
